...
def process_image(image_path, model, color_palette, conf_threshold, input_folder):
    ...
    try:
        ...
        txt_file = os.path.join(tmp_folder, "new_model_result.txt")
        with open(txt_file, 'w', encoding='utf-8') as f:
            f.write(f"文件名: {output_image_path}\n")
            f.write(f"结果: {count_pig}\n")
        # if num_instances > 0:
        #     # for idx in range(num_instances):
...
def main():
    ...
    for image_path in tqdm(image_files, desc="Processing Images"):
        process_image(image_path, model, color_palette, args.conf_threshold, args.input_folder)
